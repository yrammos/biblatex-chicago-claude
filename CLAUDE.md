...
- Do not populate the following fields:
  ...
  - reference
  - devonthink
  - URL - **exception:** populate `Url` and `Urldate` when the entry is typed `@Online` (the one type with no other locator to fall back on), when it is an online reference work (`@Inreference`/`@Reference` with `Entrysubtype = {online}`, which the package requires to carry both), or when the entry has no `Date` (in which case `Urldate` is the only dating evidence available and `Url` its necessary companion). Otherwise omit both, whether or not the entry carries a `Doi`: a printed work is cited as the printed work, and Chicago cites a DOI in preference to a URL. (An earlier form of this rule kept `Url` on any entry with no `Doi`, whatever its type. Withdrawn 2026-09-20: `src/enrich.py` never implemented it, and as prompt text it produced `.webloc`-sourced `@Article` and `@Book` entries carrying a `Url` and `Urldate` they had no use for.) This is a retention rule, not an instruction to invent an access date for a PDF that carries no URL of its own - and it applies by the entry's own fields and type, not by the source it came from: a `.webloc`-sourced page need not be typed `@Online` if the work it bookmarks is properly a book, article, etc. (see item 1 above), and conversely a PDF that turns out to be a printout of an online-only source is still typed `@Online` and keeps its `Url`/`Urldate`. If `Urldate` holds only an access date (no revision/last-modified date given by the source), it prints as "accessed <date>" and no "n.d." appears - the correct form for undated online material, and the style supplies no "n.d." of its own (compiled under the maintainer's preamble, for `@Online` and `@Inreference` alike). If the source itself states a revision or last-modified date, put that date in `Urldate` instead and add `Userd = {last modified}` (or similar) so it prints as a qualified date rather than a bare access date.
